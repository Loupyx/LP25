...
telnet_state *init_telnet_session(server *serv) {
    if (!serv || !serv->adresse || serv->port <= 0) {
        write_log("init_telnet_session : invalid server");
        return NULL;
    }
    ...
    if (s->sock < 0) {
        write_log("init_telnet_session : socket");
        free(s);
        return NULL;
    }
    ...
    if (inet_pton(AF_INET, serv->adresse, &addr.sin_addr) <= 0) {
        write_log("init_telnet_session : inet_pton");
        close(s->sock);
        free(s);
        return NULL;
    }
    ...
    if (connect(s->sock, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
        write_log("init_telnet_session : connect");
        close(s->sock);
        free(s);
        return NULL;
    }
    ...
    if (!s->telnet) {
        write_log("init_telnet_session : telnet_init");
        close(s->sock);
        free(s);
        return NULL;
    }
